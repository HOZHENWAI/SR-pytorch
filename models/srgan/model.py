...
class generator(nn.Module):
    ...
    def __init__(self, im_channels = 3,n_residual_blocks = 5, upscale_factor = 4, n_neurons_p1 = 64, n_neurons_p2 = 256):
        ...
        for u in range(self.upscale_factor/2):
            self.add_module('upscale_block_'+str(u), upscaler_block(in_channels = self.n_neurons_p1 , neurons = self.n_neurons_p2))
...
class discriminator(nn.Module):
    ...
    def __init__(self, n_residual_blocks = 7, im_channels = 3, n_neurons_p1 = 64 , n_neurons_p2 = 1024):
        ...
        self.add_module('residual_block_0', residual_block1lrelu(in_channels = self.n_neurons_p1, out_channels = self.n_neurons_p1, stride = 2))
        for n in range(1,self.n_residual_blocks):
            multiplier = 2**(n//2 +1)
            stride = 1+ (n+1)%2
            self.add_module('residual_block_'+str(n), residual_block1lrelu(in_channels = self.n_neurons_p1*multiplier, out_channels = self.n_neurons_p1*multiplier, stride = stride))
